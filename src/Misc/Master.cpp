...
Master::Master()
{
    ...
    for(int npart = 0; npart < NUM_MIDI_PARTS; ++npart) {
        vuoutpeakpart[npart] = float(1e-9);
...
void Master::vuUpdate(const float *outl, const float *outr)
{
    ...
    vu.outpeakl = float(1e-12);
    vu.outpeakr = float(1e-12);
    ...
    vu.rmspeakl = float(1e-12);
    vu.rmspeakr = float(1e-12);
    for(int i = 0; i < synth->buffersize; ++i) {
...
void Master::vuresetpeaks()
{
    ...
    vu.outpeakl    = float(1e-9);
    vu.outpeakr    = float(1e-9);
    vu.maxoutpeakl = float(1e-9);
    vu.maxoutpeakr = float(1e-9);
    vu.clipped     = 0;
    pthread_mutex_unlock(&vumutex);
}
